...
class Ia:
    ...
    def __element(self):
        ...
        if not os.path.exists(self.all_result_path + "/dump.xml"):
            self.log.warning('Failed to get xml')
            return None
    ...
    def __install_app(self, package_name, app_file_path):
        ...
        :param package_name: 应用的包名:com:x.x
        ...
        if self.queue.empty():
            ...
            while install_num < 4:
                ...
                self.log.info('install_info:%s'%install_info)
                if self.adb.is_install(package_name):
                    self.queue.put(1)
                    break
                else:
                    self.log.error('Reinstalling %s %s '%(package_name,self.device))
                    install_num += 1
            else:
    ...
    def main(self):
        ...
        apk_file = GetApkInfo.get_apkpath()
        apk = GetApkInfo.ApkInfo(apk_file)
        package_name = apk.getApkPackage()
